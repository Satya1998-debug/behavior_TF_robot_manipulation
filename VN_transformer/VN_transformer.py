...
class VNTransformer(nn.Module):
    def __init__(
        ...
        *,
        dim,
        depth,  # number of TF layers
        num_tokens = None,
        dim_feat = None, # additional features that are concatenated ( non-spatial)
        dim_head = 64, # size of attention head (total attention = dim_heads x heads)
        heads = 8, # number of heads
        dim_coor = 3,  # dimension of the coodinate system
        reduce_dim_out = False, # we dont want to use VN-Invariant features
        bias_epsilon = 0.,
        l2_dist_attn = False,
        flash_attn = False,
        ...
        super().__init__()
        # only if we use token embeddings
        self.token_emb = nn.Embedding(num_tokens, dim) if exists(num_tokens) else None

        dim_feat = default(dim_feat, 0) # default additional features are 0
        self.dim_feat = dim_feat
        self.dim_coor_total = dim_coor + dim_feat  # total dimensions = spatial + non-spatial

        assert (int(translation_equivariance) + int(translation_invariant)) <= 1 # can be either Equivariant or Invariant
